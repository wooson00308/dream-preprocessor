...
해마가 하루치 경험을 훑으며 쓸 만한 건 장기기억으로 보내고, 잡음은 버린다.

클로드는 잠을 안 잔다. 그래서 잠을 자게 만들었다.

클로드 코드는 대화할 때마다 transcript를 이미 남기고 있다.
하지만 아무도 안 읽으면 그건 그냥 로그 파일일 뿐이다.
dream은 그 로그를 주기적으로 읽어서, 노이즈를 걷어내고, 다음 대화에서 바로 쓸 수 있는 기억으로 만든다.

이 시스템의 전제는 단순하다:
- 기록은 이미 되고 있다. 별도로 기록할 필요가 없다.
- 정제만 하면 된다. 그것도 자동으로.
- 사람은 아무것도 안 해도 된다.

"기억해"라고 말하지 않아도 기억하고, "정리해"라고 말하지 않아도 정리한다.
그래서 이름이 dream이다.

KAIROS autoDream에서 영감을 받아 구현.
